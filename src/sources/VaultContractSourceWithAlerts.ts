...
export class VaultContractSourceWithAlerts extends VaultContractSource {
  ...
  protected vaultReadAlert = new DiscordAlert(
    parseInt(process.env.ALERT_VAULT_READ_COOLDOWN),
  );
  protected oracleAlert = new DiscordAlert(
    parseInt(process.env.ALERT_ORACLE_COOLDOWN),
  );
  protected oracleBalanceAlert = new DiscordAlert(
    parseInt(process.env.ALERT_ORACLE_BALANCE_COOLDOWN),
  );
  protected redemptionAlert = new DiscordAlert(
    parseInt(process.env.ALERT_REDEMPTION_COOLDOWN),
  );
  ...
  constructor(
    ...
    acc: VaultClient,
    vaultObj: any,
    z_p_f_vault_asa: boolean = false,
  ) {
    super(vaultName, acc, vaultObj, z_p_f_vault_asa);
  }
  ...
  update: () => Promise<void> = async () => {
    try {
      ...
      this.vaultReadAlert.send({
        ...
        msg:
          `🚨 Collector failed to retrieve vault state for **${this.vaultName}**\n` +
          `\`\`\`Error: ${err}\`\`\``,
      });
    }
  };
  ...
  checkOracleBalance = async (): Promise<void> => {
    try {
      ...
      const algoBalance = stdlib.formatCurrency(
        await stdlib.balanceOf(this.lastState.addresses.oracleAddress),
      );
  ...
  checkPriceChange = (): void => {
    ...
    if (timeUnchanged >= parseInt(process.env.ALERT_ORACLE_THRESHOLD)) {
      this.oracleAlert.send({
        ...
        msg: `🚨 Oracle price has not changed in ${Math.round(
          timeUnchanged / 60,
        )} minutes for **${this.vaultName}**`,
      });
    }
  };
  ...
  checkProposalTime = (): void => {
    ...
    if (
      timeWithoutProposal >= parseInt(process.env.ALERT_REDEMPTION_THRESHOLD)
    ) {
      this.redemptionAlert.send({
        username: `Keepers alert | ${process.env.NETWORK}`,
        type: `KEEPERS_NOT_PROPOSING-${this.vaultName}`,
        msg:
          `🚨 Redemption address has not been proposed since ` +
          `${lastTimeProposed.format('YYYY-MM-DD HH:mm Z')} for **${
            this.vaultName
          }**`,
      });
    }
